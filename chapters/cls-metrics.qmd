...
source("../R/_common.R")
req_pkg <- c("betacal", "discrim", "desirability2", "klaR", "mgcv", "probably", 
             "rpart", "tidymodels")
...
This example has two classes so only the probability rstimate for the first class levels is required. For 3+ classes, all of the probability columns are required. 
...
The `r pkg(yardstick)` functions accept these types of case weights but also uses basic numeric vectors. 
...
Since contains _both_ types of classification metrics, we need to use the `estimate` argument as well as the `...` to pass class probability columns: 
...
You can mix metrics types within the same model mode. You can't mix between modes: 
...
# In the end, we want the proprtions to be 70% for training and 10% for the 
# others data sets. The intial split does 70% for training, 10% for validation, 
...
The text used a naive Bayes model. For the computations, we'll use the `klaR::NaiveBayes()` function. To access a `r pkg(parsnip)` model definition, we needed to load the `r pkg(discrim)` package.

Let's make a small metric set, computed the model, and save the validation set. We'll also generate the trainined model using `fit_best()`: 
...
As seen previously, we can extract the metrics estimates...
...
... as well as the out-of-sample predictions: 
...
For classification models, the default metrics that are computed for classification models are: accuracy, the area under the ROC curve, and the Brier score. 
...
Also, these functions assume that the class probability estimate has been appropriately thresholded to convert them to hard class predictions. By default, a threshold of 1/2 is used. The `r pkg(probably)` package has [`make_two_class_pred()`](https://probably.tidymodels.org/reference/make_class_pred.html), which can be used to create alternative cutoffs. 
  ...
  mutate(
    cost = 
      case_when(
        ...
        # False positives not as much of a worry
        TRUE ~ 1.0
      )
  )
...
The `r pkg(probably)` package has functions that can estimate, validate, and apply calibration models as well as functions to visually assess the model.

The visualization functions that have names that the pattern `cal_plot_*()`. To use a simple logistic model to assess calibration in the validation set: 
...
- Logistic regression: A basic model is fit where the true class outcomes are the calibration model's outcome, and the predicted probability column is the predictor. The probability predictions from this model _should_ have improved calibration. We can estimate this with a single linear term or using spline basis functions.  
...
```{r}
#| label: logistic-spline-estimate

logistic_cal <- val_pred |>
  cal_estimate_logistic(
    truth = class,
    estimate = c(.pred_poisonous, .pred_edible)
  )
...
rule_oob_pred |> select(tries, starts_with(".pred_"))
```
