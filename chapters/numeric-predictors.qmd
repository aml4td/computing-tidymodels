...
In tidymodels, just about everything that you want to do to your predictors can be accomplished using the R formula method or, better still, the `r pkg(recipes)` package. We shall describe both. 
...
A broader discussion can be found in two blog posts [one](https://rviews.rstudio.com/2017/02/01/the-r-formula-method-the-good-parts/) and [two](https://rviews.rstudio.com/2017/03/01/the-r-formula-method-the-bad-parts/)
...
Note that the last method has its step function in the `r pkg(bestNormalize)` package; a variety of recipe extension packages can be used. A [**full set of recipe steps**](https://www.tidymodels.org/find/recipes/) for CRAN packages is available on `tidymodels.org`. 
