...
The `r pkg(rsample)` package provides many functions to facilitate resampling. For now, we’ll assume an initial split into a training and test set has been made (but see @validation-sets below for three-way splits). 
...
`r pkg(rsample)` contains several functions that resample data whose rows are thought to be statistically independent of one another. 
...
## Times Series Resampling {#sec-time-series-resampling}
...
##  Multi-Level Data {#sec-multilevel-resampling}
...
## Spatial Resampling {#sec-spatial-resampling}
...
Let's also estimate different metrics. As mentioned back in @sec-model-development, a _metric set_ is used to specify statistics for model efficacy. `fit_resamples()` has an option called `metrics` that we can use to pass in a metric set. 
