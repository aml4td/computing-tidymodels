...
source("../R/_common.R")
# fmt: skip
req_pkg <- c("Cubist", "mirai", "parallelly", "probably", "rules", "spatialsample", 
             "tidymodels", "tidysdm")
```
...
chi_rs |>
  ggplot(aes(y = id)) +
  geom_segment(
    aes(x = fit_start, xend = fit_stop, yend = id),
    col = "grey",
    linewidth = 1
  ) +
  geom_segment(
    aes(x = perf_start, xend = perf_stop, yend = id),
    col = "red",
    linewidth = 3
  ) +
  labs(y = NULL, x = "Date")
...
orth_rs <-
  ...
  mutate(
    num_subjects = map_int(splits, ~ vec_unique_count(assessment(.x)$Subject))
  )
...
Alternatively, the more recent  `r pkg(mirai)` package for parallel processing can also be used for additional efficiency. 
...
concrete_ext_res <- fit_resamples(
  rules_spec,
  compressive_strength ~ .,
  resamples = concrete_rs,
  control = ctrl
