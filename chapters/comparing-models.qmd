...
#| echo: true
req_pkg <- c("bestNormalize", "broom.mixed", "C50", "discrim", "embed", 
             "emmeans", "future.mirai", "glmnet", "klaR", "multcomp", "rules", 
             "splines2", "tidymodels", "tidyposterior")
...
Using `lme4::lmer()` to fit the model, we specify `pipeline` as the fixed effect and designate a random effect due to resampling as `(1|id)`, where `1` symbolizes the intercept. The results are: 
...
We’ll show the latter now by first defining constrats of the parameter estimates that provide all three pairwise comparisons.
...
# The defaults that mcp() uses has the opposite order than the books' main
...
For bootstrap confidence intervals, there is a helper function `rsample::int_pctl()` that has methods that can work for resampled objects (e.g., `tune::fit_resamples()`) as well as the outputs from the various `tune_*()` functions. 

As an example, to get the bootstrap interval for the boosting model: 
...
By default, this will compte intervals for every tuning parameter candidate and metric in the data (but there are arguments that can be used to restrict the computations to a smaller set).
...
metric_diff <- function(split) {
  ...
  tibble::tibble(term = "Boosting vs Logistic", estimate = diff(est$.estimate))
}
