...
From these siz candidates, the smallest Brier score was `r round(show_best(initial_res, metric = "brier_class", n = 1)$mean, 3)`, a mediocre value: 
...
bo_res %>% 
  ...
  geom_point() + 
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 1 / 4, alpha = 1 / 4) + 
  labs(x = "Iteration", y = "Brier Score (CV)")
```
...
use a different data set for now
