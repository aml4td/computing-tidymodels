...
options(pillar.advice = FALSE, pillar.min_title_chars = Inf, pillar.sigfig = 3)

source("../R/_common.R")
req_pkg <- c("brulee", "Cubist", "patchwork", "scales", "splines2", "tidymodels")
```

For this chapter, you'll need `r pkg_list(req_pkg)`. You can install them via: 
...
The data set is pre-compiled into a binary format R uses (called "RData format" here). It is in the `RData` directory. 
...
There are a lot of ways that you can examine the contents of an object. `View()` is good for data frames; in the RStudio IDE, it opens a spreadsheet-like viewer. `str()` (short for "structure") shows more details about the object, such as classes, but can be a bad choice if you have >50 columns in the data (or if it is a long list or similar). We'll use that: 
...
We can see that there are `r format(nrow(deliveries), big.mark = ",")` data points and `r ncol(deliveries)` columns and their types. 

Note that the `day` column is a _factor_. This is the preferred way to represent most categorical data (for modeling, at least). A factor catalogs the possible values of the data and stores those _levels_. That is important when we convert categorical predictors to "dummy variables" or "indicators" and similar operations.  
...
The random number stream is like a river. If we want to see the same things in your journey down the river, you must get in at the same exact spot. The seed is like the location where you start a journey (that is always the same). 
...
We used a value of 991 to set the seed: 
...
This object records which rows of the original data go into the training, validation, or test sets. The printed output shows you those as `<train/val/test/total>`. 
...
We will mostly work with the training set of `r format(nrow(delivery_train), big.mark = ",")` deliveries. That's what we'll use to explore the data, fit models, and so on. 
...
delivery_dist <- 
  ...
  # confidence band around the data; too much information to put into one plot. 
...
delivery_time_day <- 
  ...
  # With `col = day`, the trends will be estimated for each value of 'day'.
...
The methods that we used to compute the effects of the `item_*` columns are more complicated. We must make probabilistic assumptions about the data if we want to get something like a confidence interval. Alternatively, we could use the bootstrap resampling method to estimate the standard error of some statistic and use that to compute the interval.

First, we make a function that takes some data and computes our statistics of interest. It assumes `x` is the entire data set with the delivery time column and each of the item columns.
...
`rsample::int_pctl()` can consume these results and produce an interval for each item column. 
...
The analyses in this section defined a model pipeline, fit it to the training set, and then measured performance using the validation set. 

We'll review the three evaluated models and describe how those computations were done.

Before we get started, we need to specify how to measure model effectiveness. The materials use the mean absolute error (MAE). To specify this _performance metric_, you can use the `yardstick::metric_set()` function and give it the function names for specific metrics (like the `mae()` function): 
...
The first argument to step functions is the variables that should be affected by the function. We can use any `r pkg(dplyr)` selector such as `everything()` or the bare column names. Here, we want to change every factor column that was the role of "predictor". To do this, recipes have an extended set of selector functions for this purpose. 
...
The next recipe step probably is not needed for this data set but automatically using it is not problematic. What happens if there is a factor level that occurs very infrequently? It is possible that this will only be observed in the validation or test set. `step_dummy()` will make a column for that factor level since it knows it exists but the training set will have all zeros for this column; it has zero variance. We can screen these out using `step_zv()`: 
...
To fit all of this to the training set, we can use the `fit()` function. This executes the recipe on the data set then passes the appropriate data to `stats::lm()`: 
...
One helpful function is `tidy()`. It is designed to return the object results in a rational, helpful way. In our case, the `tidy()` method for an `lm` object gives us a nice data frame back with information on the fitted coefficients: 
...
Another supporting function is `augment()`. It can take a model object and data set and attach the prediction columns to the data frame. Essentially, this is a nicer version of `predict()`. Let' predict the validation set: 
...
Since we are treating this as if it were resampling, we can use the `fit_resamples()` function to do a lot of the manual work that we just showed. We'll add a _control object_ to the mix to specify that we want to retain the validation set predictions (and our original workflow).  
...
This is pretty simple and demonstrates that, after some initial code, the process of fitting different models does not require huge changes to your scripts. 
...
The `tidy()` method is also helpful here. It contains all of the rules and corresponding regression models. 

Let's get these values for the second rule in the fourth ensemble:
...
We'll use the `r pkg(brulee)` package. This uses the `torch` software to fit the model. We'll specify a few different tuning parameters but only tune the number of hidden units (for now). To mark _any_ parameter for tuning, we pass the `tune()` function to an argument: 
...
This model requires the conversion to dummy variables but does not require features to handle nonlinear trends and interactions. One additional preprocessing step that is required is to put the predictors in the same units (e.g., not miles or hours). There are a few ways to do this. We will center and scale the predictors using `step_normalize()`: 
...
# The model initializes the parameters with random numbers:
set.seed(388)
nnet_res <-
...
We can also return the parameter with the numerically best results^[As impressive as the `torch` ecosystem is, it is not as optimized for reproducibility as much as possible. These results may vary from run to run due to the inability to fix some of the random numbers used and their use of different numerical tolerances across operating systems.]: 
...
TODO For `collect_predictions()` and `cal_plot_regression()`, their default mode is the return results for all tuning parameter combinations. Both of these functions have an argument called `parameters` that can be used to filter the results. For example, here is the calibration plot for the optimal number of hidden units: 
...
cal_plot_regression(nnet_res, parameters = best_hidden_units, alpha = 1 / 3)
```

There are two options if we need a model fit on the training set. If the numerically best parameter is best (i.e., smallest MAE), then `fit_best()` is the easiest approach. Alternatively, you can choose the exact tuning parameter values you desire and _splice_ them into the model to replace the current values of `tune()`. To do this, there is a `finalize_workflow()` function. It takes a data frame with one row and columns for each tuning parameter. Here's an example where we decide that 10 hidden units is best^[If you are wondering how we get the seed values, I use `sample.int(1000, 1)` to generate random seeds on the fly.]: 
...
As you'll see in a minute, the function `cal_apply()` calibrates new predictions.  
 
## Test Set Results

As with `best_fit()`, there are two ways to go about predicting the test set.
...
There is a shortcut for the first three commands. A function called `last_fit()` takes our initial split object and automatically does the rest (but not calibration yet): 
