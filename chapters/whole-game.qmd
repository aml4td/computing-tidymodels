...
The data set is pre-compiled into a binary format R uses (called "RData format" here). It is in the `RData` directory. There is also a `csv` version in the `delimited` directory. 
...
First, we make a function that takes some data and [computes our statistics of interest](https://rsample.tidymodels.org/reference/int_pctl.html#arguments). It assumes `x` is the entire data set with the delivery time column and each of the item columns.
...
`rsample::int_pctl()` can consume these results and produce an interval for each item column^[You can see another example using `rsample::`int_pctl()` on [tidymodels.org](https://www.tidymodels.org/learn/statistics/bootstrap/).]. 
...
The analyses in this section defines a model pipeline, fits it to the training set, and then measures performance using the validation set. We'll review the three evaluated models and describe how those computations were done.
...
The first argument to step functions is the variables that should be affected by the function. We can use any `r pkg(dplyr)` [selector](https://dplyr.tidyverse.org/reference/select.html) such as `everything()` and/or the bare column names. Here, we want to change every factor column that was the role of "predictor". To do this, recipes have an [extended set of selector functions](https://recipes.tidymodels.org/reference/selections.html) for this purpose. 
...
Another valuable supporting function is `augment()`. It can take a model object and data set and attach the prediction columns to the data frame. Essentially, this is an upgraded version of `predict()`. Let' predict the validation set: 
...
`probably::cal_plot_regression()` automatically selects the best tuning parameter combination based on the _first_ metric in the metric set object. It plots a single set of predictions. For example, here is the calibration plot for the optimal number of hidden units: 
...
Note that the facet title shows the same configuration code (``r best_hidden_units$.config[1]``) as the one given by `select_best()`.
