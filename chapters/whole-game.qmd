...
You'll need `r pkg_list(req_pkg)` for this chapter. You can install them via: 
...
The data set is pre-compiled into a binary format R uses (called "RData format" here). It is in the `RData` directory. A `csv` version is also in the `delimited` directory. 
