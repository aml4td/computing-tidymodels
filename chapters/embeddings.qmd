...
Since we used `num_comp = 2`, 2 new features were generated. 
...
The individual loads are different, and components one and two are swapped between invocations with different seeds: 
...
This might not cause a difference in performance when the features are used in a predictive model, but if the model uses slopes and intercepts, the values will be different each time it is run. 
...
The shrunken version of this step has an additional argument that is the fraction of the complete solutions called `threshold`: 
