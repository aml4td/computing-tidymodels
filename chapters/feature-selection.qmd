...
#| eval: false
#| echo: true
req_pkg <- c("tidymodels", "rpart", "partykit")
...
drug_rs <- vfold_cv(drug_train, repeats = 3, strata = class)
```
...
There are a few different data frames for these data. We'll merge two and clean up and shorten the variable names, then a 4:1 split of the data:
...
When we plot the parameter choices over iterations, we see that the SVM cost, correlation threshold, and kernel parameter (`rbf_sigma`) all converge to specific ranges. The number of predictors retained fluctuates, and a few choices could be used. 
