...
If you plan to do anything beyond fixing a typo, the best thing you can do is to [open an issue](https://github.com/aml4td/computing-tidymodels/issues) and discuss changes before you spend a lot of time doing them. 
...
├── computing-tidymodels.Rproj  # Rstudio IDE file
...
#| label: contrib-list
#| include: false
use_tidy_contrib <- function(
  repo_spec = "https://github.com/aml4td/computing-tidymodels",
  from = NULL,
  to = NULL
) {
  require("usethis")
  ...
  contrib_link <- glue::glue(
    "[&#x0040;{contributors}](https://github.com/{contributors})"
  )
  contrib_markup <- glue::glue_collapse(
    contrib_link,
    sep = ", ",
    last = ", and "
  ) +
...
if (inherits(contribs, "try-error")) {
  contribs <- "[&#x0040;jrosell](https://github.com/jrosell)"
}
```

We would like to thank users who have made a contribution to the project: `r contribs`
